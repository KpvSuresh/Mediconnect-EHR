...
//   const [form, setForm] = useState({ name:'', value:'', units:'', refRange:'', date:'', orderingDoctor:'', lab:'Central Lab', notes:'' })
...
//     alert(`Attached ${files.length} file(s) to ${item.name}`)
//   }
...
//   function submit(){
//     if(!form.name || !form.value || !form.units || !form.date){ alert('Please fill required fields'); return }
//     onAdd({
//       id: 'l'+Math.random().toString(36).slice(2),
//       name: form.name,
//       value: form.value,
//       units: form.units,
//       refRange: form.refRange,
//       date: form.date,
...
//     setCurrentTest(item.name)
//     setCompareOpen(true)
//   }

//   const compareData = useMemo(() => {
//     if(!currentTest) return []
//     return items.filter(i => i.name === currentTest).map(i => ({ date: i.date, value: i.value }))
//   }, [items, currentTest])
...
//               <th className="th">Result Date</th>
//               <th className="th">Ordering Doctor</th>
...
//                 <td className="td">{item.refRange}</td>
//                 <td className="td">{item.date}</td>
//                 <td className="td">{item.orderingDoctor}</td>
//                 <td className="td">{item.lab}</td>
//                 <td className="td">{item.flagged ? <span className="badge bg-red-100 text-red-700">Flagged</span> : <span className="badge badge-gray">Normal</span>}</td>
//                 <td className="td">
//                   <div className="flex flex-wrap gap-2">
//                     <label className="btn btn-secondary">
//                       <input type="file" accept="application/pdf,image/*" multiple className="hidden" onChange={(e)=>handleFileUpload(e, item)} />
//                       Upload Report
//                     </label>
//                     {Array.from(item.files || []).map((file, idx) => (
//                       <button key={idx} className="btn btn-secondary" onClick={()=>openFileViewer(file)}>
//                         View {file.type.includes('pdf')? 'PDF' : 'Image'}
//                       </button>
//                     ))}
//                     <button className="btn btn-secondary" onClick={()=>openCompare(item)}>Compare</button>
//                     <button className={item.flagged? 'btn btn-secondary' : 'btn btn-danger'} onClick={()=>onFlag(item)}>{item.flagged? 'Unflag' : 'Flag'}</button>
//                   </div>
//                 </td>
//               </tr>
//             ))}
...
//             <input className="input" value={form.value} onChange={e=>setForm(f=>({...f, value:e.target.value}))} placeholder="e.g., 7.6"/>
//           </div>
//           <div>
//             <label className="label">Units*</label>
//             <input className="input" value={form.units} readOnly />
//           </div>
//           <div>
//             <label className="label">Reference Range</label>
//             <input className="input" value={form.refRange} readOnly />
//           </div>
//           <div>
...
//               {['Central Lab','City Diagnostics','PathCare'].map(l => <option key={l} value={l}>{l}</option>)}
//             </select>
//           </div>
...
//       {/* Compare Modal */}
//       <Modal open={compareOpen} onClose={()=>setCompareOpen(false)} title={currentTest? `Compare: ${currentTest}` : 'Compare'}>
//         <div className="space-y-4">
//           <TrendChart data={compareData} title={`Trend · ${currentTest || ''}`} />
//           <div className="text-sm text-slate-600">Comparing all results of the same test name across time for this patient.</div>
//         </div>
//       </Modal>

//       Viewer Modal
//       <Modal open={viewerOpen} onClose={closeFileViewer} title="Report Viewer">
//         {viewerFileUrl ? (
//           <div className="w-full h-[70vh]">
//             <iframe src={viewerFileUrl} className="w-full h-full border rounded" />
//           </div>
//         ) : (
...
//   // Standard form state (All original inputs preserved)
...
//                 <td className="td">{item.lab}</td>
//                 <td className="td">
//                     {item.flagged ? <span className="badge bg-red-100 text-red-700">Flagged</span> : <span className="badge badge-gray">Normal</span>}
//                 </td>
//                 <td className="td">
//                   <div className="flex flex-wrap gap-2">
//                     {/* Upload Logic */}
...
//       {/* Add Lab Modal - All original fields preserved */}
//       <Modal open={open} onClose={() => setOpen(false)} title="Add Lab Result">
//         <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
//           <div>
//             <label className="label">Test Name*</label>
...
//       {/* Simplified PDF/Image Viewer Modal */}
